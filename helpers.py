...
from route import SegmentName
from typing import List
from xattr_cache import getxattr
...
FOOTAGE_PATH = "./data/media/0/realdata/"
...
def is_valid_segment(segment):
  try:
    segment_to_segment_name(FOOTAGE_PATH, segment)
    return True
...
def preserved_routes():
  dirs = listdir_by_creation(FOOTAGE_PATH)
  preserved_segments = get_preserved_segments(dirs)
  return sorted(preserved_segments, reverse=True)

def has_preserve_xattr(d: str) -> bool:
  return getxattr(os.path.join(FOOTAGE_PATH, d), PRESERVE_ATTR_NAME) == PRESERVE_ATTR_VALUE
...
# temp functions for pc function
