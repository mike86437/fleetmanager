...
def full(cameratype, route):
  ...
  vidlist = "|".join(fleet.FOOTAGE_PATH + "/" + segment + "/" + file_name for segment in fleet.segments_in_route(route))
...
def fcamera(cameratype, segment):
  ...
  file_name = fleet.FOOTAGE_PATH + "/" + segment + "/" + cameratype + (".ts" if cameratype == "qcamera" else ".hevc")
...
def footage():
  ...
  for route_path in route_paths:
    input_path = fleet.FOOTAGE_PATH + route_path + "--0/qcamera.ts"
    output_path = fleet.FOOTAGE_PATH + route_path + "--0/preview.gif"
...
def preserved():
  ...
  for segment in segments:
    input_path = fleet.FOOTAGE_PATH + segment + "/qcamera.ts"
    output_path = fleet.FOOTAGE_PATH + segment + "/preview.gif"
    fleet.video_to_img(input_path, output_path)
...
def addr_input():
  preload = "111 Main St", "222 Main St", "333 Main St", "333 Main St", "444 Main St"
  SearchInput = 0
  token = 0
  s_token = 0
